...
**Game stack (local dev):** `pnpm dev:game` starts Postgres, game-server, and game-hub. Use `pnpm dev:game-hub` or `pnpm dev:game-server` to run one service only.
...
- Schema sync runs automatically when `dev:game` starts the server (admin `sync-db` until startup bootstrap lands in P0).
